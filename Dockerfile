...
#镜像的制作人
MAINTAINER mqz
ENV APP_NAME="nacos-provider-a-0.0.1"
#定义jar包名称 给个默认值
ARG JAR_NAME=${APP_NAME}
#此处必须这么指定，ARG参数在最终的ENTRYPOINT执行时会出现问题
ENV FILE_NAME="${JAR_NAME}.jar"
#APPLICATION_NAME是pom文件传过来的参数
ENV WORK_DIR="/home/mqz/nacos-project/${APPLICATION_NAME}/${APP_NAME}"
ENV JAVA_MS="1G"
...
EXPOSE 12340
#健康检查
HEALTHCHECK --start-period=60s --interval=60s --timeout=60s --retries=10 CMD curl --fail localhost:9999/actuator/health || exit 1
